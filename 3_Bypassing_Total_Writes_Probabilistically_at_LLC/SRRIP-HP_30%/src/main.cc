...
{
    string trace_name=""; // name of the trace for the csv file
...
///////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
    //calculte the writes and interV, intraV
    uint64_t set_total[LLC_SET]={};
    uint64_t set_highest[LLC_SET]={};
    uint64_t highest_write=0;
    long double IntraV=0, InterV=0;
    long double Write_avg=0;

    for (uint32_t i=0; i<LLC_SET; i++) {
        uint64_t count=0;
        uint64_t set_high=0;
        for (uint32_t j=0; j<LLC_WAY; j++) {
            count+=LLC_WRITE_SET[i][j];
            highest_write += LLC_WRITE_SET[i][j];
            set_high= max(set_high, LLC_WRITE_SET[i][j]);
        }
        set_total[i]=count;
        set_highest[i]=set_high;
    }


    Write_avg = (highest_write/(LLC_SET*LLC_WAY));

    
    //calculating the total access, hit, miss of LLC for single core only

    uint64_t total_access_LLC=0, total_hit_LLC=0, total_miss_LLC=0;
    uint64_t total_access_L1D=0, total_hit_L1D=0, total_miss_L1D=0;
    uint64_t total_access_L1I=0, total_hit_L1I=0, total_miss_L1I=0;
    uint64_t total_access_L2=0, total_hit_L2=0, total_miss_L2=0;

    for(uint32_t i=0; i<NUM_TYPES; i++)
    {
        total_access_LLC+=(&uncore.LLC)->roi_access[0][i],
        total_hit_LLC+=(&uncore.LLC)->roi_hit[0][i];
        total_miss_LLC+=(&uncore.LLC)->roi_miss[0][i];

        total_access_L1D+=(&ooo_cpu[0].L1D)->roi_access[0][i],
        total_hit_L1D+=(&ooo_cpu[0].L1D)->roi_hit[0][i];
        total_miss_L1D+=(&ooo_cpu[0].L1D)->roi_miss[0][i];

        total_access_L1I+=(&ooo_cpu[0].L1I)->roi_access[0][i],
        total_hit_L1I+=(&ooo_cpu[0].L1I)->roi_hit[0][i];
        total_miss_L1I+=(&ooo_cpu[0].L1I)->roi_miss[0][i];

        total_access_L2+=(&ooo_cpu[0].L2C)->roi_access[0][i],
        total_hit_L2+=(&ooo_cpu[0].L2C)->roi_hit[0][i];
        total_miss_L2+=(&ooo_cpu[0].L2C)->roi_miss[0][i];

        
    }

    long double hit_rate_LLC = (float)total_hit_LLC/total_access_LLC;
    string name="";
    for(uint32_t i=0; i<trace_name.size(); i++ )
    {
        if(i>34)
        name+=trace_name[i];
    }
    long double ipc=((float) ooo_cpu[0].finish_sim_instr / ooo_cpu[0].finish_sim_cycle);
    // cout<<"trace name " <<name <<endl;   
    // cout<< "total access at L1D: " << total_access_L1D <<endl;
    // cout<< "total hit at L1D: " << total_hit_L1D <<endl;
    // cout<< "total miss at L1D: " << total_miss_L1D <<endl;
    // cout<< "total access at L1I: " << total_access_L1I <<endl;
    // cout<< "total hit at L1I: " << total_hit_L1I <<endl;
    // cout<< "total miss at L1I: " << total_miss_L1I <<endl;
    // cout<< "total access at L2: " << total_access_L2 <<endl;
    // cout<< "total hit at L2: " << total_hit_L2 <<endl;
    // cout<< "total miss at L2: " << total_miss_L2 <<endl;
    // cout<< "total access at LLC: " << total_access_LLC<<endl;
    // cout<< "total hit at LLC: " << total_hit_LLC<<endl;
    // cout<< "total miss at LLC: " << total_miss_LLC << endl;
    // cout<< "total writes: "<< highest_write <<endl;
    // cout<< "Write average: "<< Write_avg <<endl;
    // cout<< "Inter V: " <<InterV <<endl;
    // cout<< "Intra V: " <<IntraV <<endl;
    // cout<< "Total cycles" << current_core_cycle[0] - ooo_cpu[0].begin_sim_cycle <<endl;
    // cout<< "IPC: " << ipc<<endl;
    
    std::ofstream csv_file;
    csv_file.open("/home/deepesh/Assignment/results/Q3SRRIP_HP_30PERC.csv", std::ios::out | std::ios::app);
    csv_file << name << ";" << ipc << ";"<< total_miss_L1D  <<";" <<  total_miss_L1I  <<";" << total_miss_L2 << ";" << total_miss_LLC << ";" << hit_rate_LLC << ";" << highest_write << ";"<< endl;
    csv_file.close();
