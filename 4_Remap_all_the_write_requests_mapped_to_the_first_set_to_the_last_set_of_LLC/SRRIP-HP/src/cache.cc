...
uint64_t LLC_WRITE_SET[LLC_SET][LLC_WAY] ;
...
void CACHE::handle_fill()
{
        ...
        if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) 
        {
            ...
            {
              if(set==0)
                way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, LLC_SET-1, block[LLC_SET-1], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
              else 
                ...
                way = find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
                    ...
                    if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
                        ...
                        {
                          if(set==0)
                            llc_update_replacement_state(fill_cpu, LLC_SET-1, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, 0, MSHR.entry[mshr_index].type, 0);
                          else 
            ...
            if (do_fill)
            {
                ...
                if (cache_type == IS_LLC)
                {
                  cpu = fill_cpu;
                  if(set==0)
                    MSHR.entry[mshr_index].pf_metadata = llc_prefetcher_cache_fill(MSHR.entry[mshr_index].address<<LOG2_BLOCK_SIZE, LLC_SET-1, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, block[LLC_SET-1][way].address<<LOG2_BLOCK_SIZE, MSHR.entry[mshr_index].pf_metadata);
                  else 
                ...
                {
                  if(set==0)
                    llc_update_replacement_state(fill_cpu, LLC_SET-1, way, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].ip, block[LLC_SET-1][way].full_addr, MSHR.entry[mshr_index].type, 0);
                  else 
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) 
    {
        ...
        { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_LLC) 
            {
                if(set==0)
                {
                  llc_update_replacement_state(writeback_cpu, LLC_SET-1, way, block[LLC_SET-1][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1);
                  LLC_WRITE_SET[LLC_SET-1][way]++;
                }

                else
                {
                  llc_update_replacement_state(writeback_cpu, set, way, block[set][way].full_addr, WQ.entry[index].ip, 0, WQ.entry[index].type, 1);
                  LLC_WRITE_SET[set][way]++;
                }
            ...
            else 
            {
                  ...
                  {
                    if(set==0)
                      way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, LLC_SET-1, block[LLC_SET-1], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type);
                    else 
                  ...
                  if (do_fill) 
                  {
                      ...
                      if (cache_type == IS_LLC)
                      {
                          cpu = writeback_cpu;
                          if(set==0) 
                            WQ.entry[index].pf_metadata =llc_prefetcher_cache_fill(WQ.entry[index].address<<LOG2_BLOCK_SIZE, LLC_SET-1, way, 0, block[LLC_SET-1][way].address<<LOG2_BLOCK_SIZE, WQ.entry[index].pf_metadata);
                      ...
                      {
                        if(set==0)
                          llc_update_replacement_state(writeback_cpu, LLC_SET-1, way, WQ.entry[index].full_addr, WQ.entry[index].ip, block[LLC_SET-1][way].full_addr, WQ.entry[index].type, 0);
                        else 
		    ...
		    if(cache_type == IS_L1I)
		      ...
		      l2c_prefetcher_operate(block[set][way].address<<LOG2_BLOCK_SIZE, RQ.entry[index].ip, 1, RQ.entry[index].type, 0);
                    else if (cache_type == IS_LLC)
                        {
                          cpu = read_cpu;
                          if(set==0)
                            llc_prefetcher_operate(block[LLC_SET-1][way].address<<LOG2_BLOCK_SIZE, RQ.entry[index].ip, 1, RQ.entry[index].type, 0);
                          else 
                ...
                if (cache_type == IS_LLC) {
                  if(set==0)
                    llc_update_replacement_state(read_cpu, LLC_SET-1, way, block[LLC_SET-1][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1);
                  else 
      ...
      if (prefetch_cpu == NUM_CPUS)
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            if (way >= 0) { // prefetch hit
                ...
                if (cache_type == IS_LLC) {
                  if(set==0)
                    llc_update_replacement_state(prefetch_cpu, LLC_SET-1, way, block[LLC_SET-1][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1);
                  else 
		    ...
		    if (cache_type == IS_L1D)
		      l1d_prefetcher_operate(PQ.entry[index].full_addr, PQ.entry[index].ip, 1, PREFETCH);
                    ...
                    else if (cache_type == IS_LLC)
                    {
                        cpu = prefetch_cpu;
                        if(set==0)
                            PQ.entry[index].pf_metadata = llc_prefetcher_operate(block[LLC_SET-1][way].address<<LOG2_BLOCK_SIZE, PQ.entry[index].ip, 1, PREFETCH, PQ.entry[index].pf_metadata);
                        else 
